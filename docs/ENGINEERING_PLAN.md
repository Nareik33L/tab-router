...
(default 2) and startup URL. Routes are provisioned automatically. Never prompt
for SOCKS/HTTP credentials or a provider account. `--fresh` does not delete
optional leftover provider configuration.
